...
export const urlSchema = z.string().trim().url("Invalid URL").optional().nullable();
...
// Common date fields
export const dateSchema = z.coerce.date().optional().nullable();

// Common ID fields
export const objectIdSchema = z.string().trim().min(1, { message: "ID is required" });

// Common array fields
export const stringArraySchema = z.array(z.string()).optional().default([]);

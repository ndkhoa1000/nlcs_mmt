...
import { invitedUserJoinOrganizationMutationFn } from "@/lib/api";
...
const InviteUser = () => {
  ...
  const { mutate, isPending: isLoading } = useMutation({
    mutationFn: invitedUserJoinOrganizationMutationFn,
  });
  ...
  const handleSubmit = (e: { preventDefault: () => void }) => {
    ...
    mutate(inviteCode, {
      onSuccess: (data) => {
        ...
        navigate(`/organization/${data?.member?.orgId}`);
        toast({
          title: "Success",
          description: `Join Organization Successfully.`,
          variant: "success",
        });
      },
      onError: (error: CustomError) => {
        console.log('error Join org:', error);
          ...
          <Card>
            <CardHeader className="text-center">
              <CardTitle className="text-xl">
                Hey there! You're invited to join an Open Heart Organization!
              </CardTitle>
              <CardDescription>
                Looks like you need to be logged into your Open Heart account to
